...
def main():
    ...
    with server.auth.sign_in(auth):
        ...
        # 워크북 탐색 — 전체 목록에서 이름 매칭
        all_workbooks, _ = server.workbooks.get()
        print(f"\n접근 가능한 워크북 목록 ({len(all_workbooks)}개):")
        ...
        wb = None
        for w in all_workbooks:
            if WORKBOOK_NAME.lower() in w.name.lower() or w.name.lower() in WORKBOOK_NAME.lower():
                wb = w
                break

        if not wb:
            raise RuntimeError(
                f"워크북 '{WORKBOOK_NAME}' 을 찾을 수 없습니다. 위 목록에서 정확한 이름을 확인하세요."
            )
        ...
        def get_view(sheet_name):
            for v in wb.views:
                if sheet_name.lower() in v.name.lower():
                    return v
        ...
        def download_csv(view):
            ...
            raw = b''.join(view.csv)
            return raw.decode('utf-8-sig')  # BOM 제거

        # 다운로드
        print(f"\n⬇️  Traffic 다운로드: {TRAFFIC_SHEET}")
        trf_csv = download_csv(get_view(TRAFFIC_SHEET))

        print(f"⬇️  Revenue 다운로드: {REVENUE_SHEET}")
        rev_csv = download_csv(get_view(REVENUE_SHEET))

...
def main():
    ...
    with server.auth.sign_in(auth):
        ...
        # ── 워크북 내 전체 뷰(워크시트) 목록 가져오기 ──────────────────────────
        resp2 = requests.get(f"{base}/workbooks/{wb_id}/views",
                             headers=headers)
        resp2.raise_for_status()
        views_data = resp2.json().get('views', {}).get('view', [])
        print(f"\n시트 목록 ({len(views_data)}개):")
        for v in views_data:
            print(f"  - [{v['id']}] {v['name']}")
        ...
        def get_view_id(sheet_name):
            for v in views_data:
                if sheet_name.lower() in v['name'].lower():
                    return v['id'], v['name']
            available = [v['name'] for v in views_data]
